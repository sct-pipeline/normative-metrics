...
#
# Manual usage (uncommment line with $PATH_RESULTS definition):
...
for metric in ${DWI_METRICS_TO_PROCESS[@]};do
  if [[ -f dti_${metric}.nii.gz ]];then
    ...
    echo "File ${PATH_RESULTS}/data/${SUBJECT}/dwi/dti_${metric}.nii.gz does not exist."
  fi
done
...
for metric in ${MT_METRICS_TO_PROCESS[@]};do
  ...
  if [[ -f ${metric_lowercase}.nii.gz ]];then
    ...
    echo "File ${PATH_RESULTS}/data/${SUBJECT}/anat/${metric_lowercase}.nii.gz does not exist."
  fi
done
